...
export function SiteSections({ content, onMenuClick }: { content: SiteContent; onMenuClick?: () => void }) {
  ...
  return (
    <>
      ...
      <button 
        className="whatsapp-float flex flex-col items-center justify-center gap-1 group" 
        onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
        aria-label="Voltar ao topo"
      >
        <svg viewBox="0 0 24 24" className="w-6 h-6 fill-white group-hover:scale-110 transition-transform"><path d="M12 4l-8 8h16l-8-8z" /></svg>
        <span className="text-[8px] font-bold text-white uppercase tracking-tighter opacity-0 group-hover:opacity-100 transition-opacity">Topo</span>
      </button>

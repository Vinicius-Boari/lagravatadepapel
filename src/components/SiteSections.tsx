  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (c < window.innerHeight * 1.5 && heroImgsRef.current) {
        ...
        if (imgs[0]) imgs[0].style.transform = `rotateY(-6deg) rotateZ(-6deg) translateY(${c * 0.08}px)`;
        if (imgs[1]) imgs[1].style.transform = `translate(-50%, -50%) rotateZ(2deg) translateY(${c * 0.04}px)`;
        if (imgs[2]) imgs[2].style.transform = `rotateY(6deg) rotateZ(5deg) translateY(${c * 0.06}px)`;
      }
    };

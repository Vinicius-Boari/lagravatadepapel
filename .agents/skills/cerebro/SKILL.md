...
### 4. Cyber Security (Defensive Core)
- **Definição**: Garantia de integridade, confidencialidade e disponibilidade dos dados e sistemas.
- **Domínios**: Secure Coding (OWASP Top 10), Hardening (CI/CD), Cryptography, Identity & Access Management (IAM).
- **Tech Stack**: Helmet.js, Supabase RLS, Snyk, SonarQube, GitHub GHSA.
- **Habilidades Específicas**:
    - **Proteção de Input**: Sanitização rigorosa e validação de dados (Zod/Valibot).
    - **Autenticação Segura**: Implementação de MFA, sessões seguras e proteção contra brute-force.
    - **Segurança de Dados**: Row Level Security (RLS), consultas parametrizadas e criptografia em repouso.
    - **Hardening de Cabeçalhos**: Configuração de CSP, HSTS, X-Frame-Options.
    - **Supply Chain**: Gestão segura de dependências e análise de vulnerabilidades (SBOM).

### 5. Aprendizagem Autônoma (Módulo Core)
...
### 6. Pesquisa Inteligente (Módulo de Expansão v1.0)
